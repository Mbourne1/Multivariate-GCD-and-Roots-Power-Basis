...
switch SETTINGS.BOOL_ALPHA_THETA
    case 'y'
        ...
        str1 = sprintf('Condition S(f(x),g(x)) : %2.4e', cond(BuildSylvesterMatrix_Respective(fxy,gxy,1,1)));
        str2 = sprintf('Condition S(f(w),alpha*g(w)) : %2.4e', cond(BuildSylvesterMatrix_Respective(fww,alpha.*gww,1,1)));
...
fullFileName = 'Results_Preproc.txt';


if exist('Results_Preproc.txt', 'file')
    fileID = fopen('Results_Preproc.txt','a');
