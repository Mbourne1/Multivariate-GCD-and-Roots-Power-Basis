...
nCoefficients_f = nchoosek(m+2,2);
nCoefficients_g = nchoosek(n+2,2);
...
PartOne = ...
    [
        ones(nCoefficients_f,1) ...
        zeros(nCoefficients_f,1) ...
        -1 .* i1s_f(1:nCoefficients_f) ...
        -1 .* i2s_f(1:nCoefficients_f) ...
        zeros(nCoefficients_f,1)
    ];
...
PartTwo = ...
    [
        ones(nCoefficients_g,1) ...
        zeros(nCoefficients_g,1) ...
        -1 .*  i1s_g(1:nCoefficients_g) ...
        -1 .*  i2s_g(1:nCoefficients_g) ...
        -1 .* ones(nCoefficients_g,1)
    ];

PartThree = ...
    [
        zeros(nCoefficients_f,1) ...
        -1.*ones(nCoefficients_f,1) ...
        i1s_f(1:nCoefficients_f) ...
        i2s_f(1:nCoefficients_f) ...
        zeros(nCoefficients_f,1)
    ];
...
PartFour = ...
    [
        zeros(nCoefficients_g,1) ...
        -1.*ones(nCoefficients_g,1) ...
        i1s_g(1:nCoefficients_g) ...
        i2s_g(1:nCoefficients_g) ...
        ones(nCoefficients_g,1)
    ];
...
lambda_vec  = fxy_vec(1:nCoefficients_f); 
rho_vec     = fxy_vec(1:nCoefficients_f);

gxy_vec     = GetAsVector(abs(gxy_matrix));
mu_vec      = gxy_vec(1:nCoefficients_g);
tau_vec     = gxy_vec(1:nCoefficients_g);
